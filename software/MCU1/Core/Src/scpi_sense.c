...
scpi_result_t SCPI_SenseAverageCount(scpi_t * context)
{
	...
	if(paramAVER.special)
	{
		...
		{
		case SCPI_NUM_MIN: snprintf(tx_data, SPI4_BUFFER, "SENS:AVER:COUN %d", 1); SPI4_Transmit(&tx_data,1000); break;
		case SCPI_NUM_MAX: snprintf(tx_data, SPI4_BUFFER, "SENS:AVER:COUN %d", 256); SPI4_Transmit(&tx_data,1000); break;
	...
	else
	{
		if(SCPI_UNIT_NONE == paramAVER.unit || SCPI_UNIT_UNITLESS == paramAVER.unit)
		{
			...
			{
				snprintf(tx_data, SPI4_BUFFER, "SENS:AVER:COUN %d", (int32_t)paramAVER.content.value);
...
scpi_result_t SCPI_SenseFimpedanceRange(scpi_t * context)
{
	...
	if(paramRANG.special)
	{
		...
		{
		case SCPI_NUM_MIN: snprintf(tx_data, SPI4_BUFFER, "SENS:FIMP:RANG %d", 10); SPI4_Transmit(&tx_data,1000); break;
		case SCPI_NUM_MAX: snprintf(tx_data, SPI4_BUFFER, "SENS:FIMP:RANG %d", 100000); SPI4_Transmit(&tx_data,1000); break;
		default: SCPI_ErrorPush(context, SCPI_ERROR_ILLEGAL_PARAMETER_VALUE); return SCPI_RES_ERR;
		}
	}
	else
	{
		if(SCPI_UNIT_NONE == paramRANG.unit || SCPI_UNIT_UNITLESS == paramRANG.unit)
		{
			...
			{
				snprintf(tx_data, SPI4_BUFFER, "SENS:FIMP:RANG %d", (int32_t)paramRANG.content.value);
				SPI4_Transmit(&tx_data,1000);
				return SCPI_RES_OK;
			}
		}
